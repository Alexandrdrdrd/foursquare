...
//    private static final FOURSQUARE_URL_SORT_BY_POP = 'https://api.foursquare.com/v2/venues/explore?ll=%s,%s&&radius=%d&openNow=1&sortByPopularity=1&section=%s&limit=1&client_id=%s&client_secret=%s&v=20190425'
   ...
   /* public JsonNode findVenuesByPopularity(String[] coordinatesAndSection, Integer radius) {
        def properties = applicationProperties.getParams()
        def json_request = new URL(String.format(FOURSQUARE_URL_SORT_BY_POP
                , coordinatesAndSection[0], coordinatesAndSection[1],radius,
                properties.Client_Id
                , properties.client_secret))
        return new ObjectMapper().readTree(json_request);
    }*/
